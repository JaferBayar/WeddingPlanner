...
<body>
    ...
    <div class="lang-switcher">
        <form method="post" action="theme.php" style="display:inline">
            <input type="hidden" name="lang" value="ku">
            <button type="submit" class="lang-btn <?=$lang==='ku'?'active':''?>">کوردی</button>
        </form>
        <form method="post" action="theme.php" style="display:inline">
...
<header class="top">
    ...
    <nav class="nav-actions">
        ...
        <form method="post" action="theme.php" style="margin:0"><input type="hidden" name="lang" value="ku"><button class="btn btn-lang <?=$lang==='ku'?'active':''?>" type="submit">کوردی</button></form>
...
<div class="welcome-hero">
    ...
    <div class="welcome-content">
        ...
        <h1 class="welcome-title">
            <?php
            if($isRTL && $lang === 'ar'){
                echo 'لنخطط ليومك المثالي';
            } elseif($isRTL && $lang === 'ku'){
                echo 'با پلان بۆ ڕۆژێی خەونت بدانە';
            } else {
                echo "Let's Plan Your Perfect Day";
            }
            ?>
        </h1>
        <p class="welcome-subtitle">
            <?php
            if($isRTL && $lang === 'ar'){
                echo 'اختر من خدماتنا وباقاتنا المختارة بعناية';
            } elseif($isRTL && $lang === 'ku'){
                echo 'لە نێوان خزمەتگوزاری و پاکەتە وەرزراوەکانمان هەڵبژێرە';
            } else {
                echo 'Choose from our curated services and packages below';
            }
            ?>
        </p>
        <div class="welcome-scroll" onclick="document.getElementById('services-section').scrollIntoView({behavior:'smooth'})">
            <span>
                ...
                if($isRTL && $lang === 'ar'){
                    ...
                    echo 'خزمەتگوزارییەکان ببینە';
                } else {
                    echo 'Explore Services';
                }
                ?>
            </span>
    ...
    <div class="cart-items">
        ...
        <div class="cart-item">
            ...
            <form method="post" action="index.php" style="margin:0">
                ...
                <button type="submit" class="cart-item-delete" title="<?php
                    if($isRTL && $lang === 'ar'){
                        echo 'حذف';
                    } elseif($isRTL && $lang === 'ku'){
                        echo 'سڕینەوە';
...
// Function to reload cart contents
function reloadCartPanel(){
    ...
    .catch(err => console.error('Error reloading cart:', err));
}
...
function setupAjaxCartForms(){
    ...
    function attachAjaxToForms(actionValue){
        document.querySelectorAll('form[action="index.php"] input[name="action"][value="'+actionValue+'"]').forEach(input=>{
            ...
            form.addEventListener('submit', function(e){
                ...
                .then(data => {
                    if(data && data.success){
                        ...
                        if(typeof data.cartCount !== 'undefined' && cartCountEl){
                            cartCountEl.textContent = data.cartCount;
                        }
                        // Reload the cart panel contents - THIS IS THE FIX!
                        reloadCartPanel();
                    }
                })
                .catch(()=>{});
    ...
    attachAjaxToForms('add_to_cart');
    attachAjaxToForms('add_bundle_to_cart');
}

if(document.readyState === 'loading'){
    document.addEventListener('DOMContentLoaded', setupAjaxCartForms);
} else {
    setupAjaxCartForms();
}
</script>
...
<div class="split-left">
    ...
    <div class="lang-switch">
        <form method="post" action="theme.php" style="display:inline"><input type="hidden" name="lang" value="ku"><button class="lang-btn <?=$lang==='ku'?'active':''?>" type="submit">کوردی</button></form>
    ...
    <div class="form-container">
        ...
        <div class="tagline">
            <?php
            if($isRTL && $lang === 'ar'){
                echo 'نحقق أحلام الزفاف';
            } elseif($isRTL && $lang === 'ku'){
                echo 'خەونەکانی شهيانەکەت دەکەین به ڕاست';
            } else {
                echo 'Making wedding dreams come true';
            }
            ?>
        </div>
        <div class="form-card">
            ...
            <form method="post" action="login.php">
                <div class="input-group">
                    ...
                    <input type="email" name="email" class="input-field" placeholder="<?php
                        if($isRTL && $lang === 'ar'){
                            echo 'أدخل بريدك الإلكتروني';
                        } elseif($isRTL && $lang === 'ku'){
                            echo 'ئیمەیڵەکەت بنووسە';
                        } else {
                ...
                <div class="input-group">
                    ...
                    <input type="password" name="password" class="input-field" placeholder="<?php
                        if($isRTL && $lang === 'ar'){
                            echo 'أدخل كلمة المرور';
                        } elseif($isRTL && $lang === 'ku'){
                            echo 'وشەی نهێنیت بنووسە';
                        } else {
...
<div class="split-right">
    ...
    <div class="overlay-text">
        <h2><?php
            if($isRTL && $lang === 'ar'){
                echo 'يومك المثالي يبدأ هنا';
            } elseif($isRTL && $lang === 'ku'){
                echo 'ڕۆژی خەونى شهيانت لێرە دەستپێدەکات';
            } else {
                echo 'Your Perfect Day Starts Here';
            }
        ?></h2>
        <p><?php
            if($isRTL && $lang === 'ar'){
                echo 'انضم إلى آلاف الأزواج السعداء الذين خططوا لحفل زفافهم معنا';
            } elseif($isRTL && $lang === 'ku'){
                echo 'بەشدار بە لای زۆرینەی هاوسەرگیریی دڵخۆش کە شهيانەکانیان لەگەڵ ئێمە پلان کردووە';
            } else {
                echo 'Join thousands of happy couples who planned their dream wedding with us';
            }
        ?></p>
    </div>
</div>
    ...
    <div class="form-container">
        ...
        <div class="tagline">
            ...
            if($isRTL && $lang === 'ar'){
                ...
                echo 'خەونەکانی شهيانه كه ت دەکەین ب ڕاست';
            } else {
                echo 'Making wedding dreams come true';
            }
            ?>
        ...
        <div class="form-card">
            ...
            <form method="post" action="signup.php">
                <div class="input-group">
                    ...
                    <input type="text" name="username" class="input-field" placeholder="<?php
                        if($isRTL && $lang === 'ar'){
                            echo 'أدخل اسمك الكامل';
                        } elseif($isRTL && $lang === 'ku'){
                            echo 'ناوی تەواوت بنووسە';
                        } else {
            ...
            <div class="footer-links">
                <?php
                if($isRTL && $lang === 'ar'){
                    echo 'لديك حساب بالفعل؟';
                } elseif($isRTL && $lang === 'ku'){
                    echo 'هەژمارت هەیە پێشتر؟';
                } else {
